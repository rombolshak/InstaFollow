...
class Instagram {
    ...
    protected $_endpointUrls = array(
        ...
        'user_search' => 'https://api.instagram.com/v1/users/search?q=%s&count=1&access_token=%s',
    ...
    */
    ...
    public function getAccessToken() {
        if ($this->_accessToken == null) {
            ...
            $parts = preg_split('|(?:\r?\n){2}|m', $this->_oauthToken);
            $response = json_decode($parts[count($parts)-1]);
            if (isset($response->access_token))
                $this->_accessToken = $response->access_token;
            elseif ($response->code != 200)
                throw new CHttpException($response->code, $response->error_message);            
        }
    ...
    public function getUser($id) {
        ...
        return $this->parseJson($this->_getHttpClientResponse());
    }
     ...
     * @param string $cursor. Next cursor
     */
    public function getUserFollows($id, $cursor = '') {
        ...
        $this->_initHttpClient($endpointUrl);
        $response = $this->_getHttpClientResponse();                
        return array_merge($this->_httpClient->extractHeaders($response), $this->parseJson($response)); 
    }

    /**
     * Get the list of user this user is followed by.
     * @param integer $id
     */
    ...
    /**
     * List the user who have requested this user's permission to follow
     */
    ...
    public function modifyUserRelationship($id, $action) {
        ...
        //$this->_httpClient->setHeaders(false);
    ...
    /**
     * Get a list of user who have liked this media.
     * @param integer $mediaId
     */
	...
	protected function parseJson($response) {
		try {
			...
			if (!isset($result)) {
				throw new CHttpException(400, Yii::t('InstagramApp', 'Invalid response format: response = '.$response));
			}
			else if (isset($error)) {
